...
import { ArrowLeft, RefreshCw, ClipboardCopy, ThumbsUp, Check } from "lucide-react";
...
export default function ResearchDetailPage({ params }: { params: { id: string } }) {
  ...
  // React.useを使用してparamsをアンラップ
  const researchId = React.use(Promise.resolve(params.id));
  ...
  // フィードバック待ちの場合はフィードバックページにリダイレクト
