...
""",
    "local": """
<Local>
Local search is performed on a SQLite FTS (full text search) database of stored documents. The search conditions are generated based on the following guidelines:
1. Prioritize maximizing recall (comprehensiveness) above all
2. Generate search conditions specifically for SQLite Full-Text Search (FTS)
3. Include a wide range of synonyms, equivalent terms, and related expressions for keywords
4. Include both Japanese and English keywords, connecting all with OR operators
   Example: (機械学習 OR machine learning OR ML OR 学習アルゴリズム OR learning algorithm)
5. When multiple concepts are involved, group each concept separately
6. Use AND operators only when limiting to specific fields
   Example: ((深層学習 OR deep learning OR ディープラーニング OR DL OR neural network OR ニューラルネットワーク) AND (画像認識 OR image recognition OR computer vision OR 画像理解 OR 画像分析))
7. Be sure to include common industry abbreviations, technical terms, and spelling variations
8. Output only the search condition part compatible with SQLite FTS (SQL statement not needed)
</Local>
""",
}
